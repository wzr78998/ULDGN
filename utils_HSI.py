...
def seed_worker(seed):
    ...
    if torch.cuda.is_available():
        ...
        torch.cuda.manual_seed_all(seed)  # if you are using multi-GPU.  # 如果使用多GPU，设置所有GPU的随机种子
    np.random.seed(seed)  # Numpy module.# 设置NumPy的随机种子
    random.seed(seed)  # Python random module.
    torch.backends.cudnn.benchmark = False# 关闭cuDNN的自动调优功能，以确保结果的一致性
    torch.backends.cudnn.deterministic = True# 设置cuDNN使用确定性算法，以确保结果的一致性
...
# 百分比:[0,1]float
# 返回:
# train_gt, test_gt: int标签的二维数组

    indices = np.nonzero(gt)  #标签真值非零信息   获取标签图像gt中非零像素的索引位置。
    X = list(zip(*indices)) # x,y features  将indices转换为x和y坐标的列表，并将其打包为元组的列表。
    y = gt[indices].ravel() # classes   使用indices索引标签图像gt，获取对应位置的类别信息，并将其展平为一维数组，作为标签y。
    train_gt = np.zeros_like(gt)  #创建一个与gt形状相同的零数组，并赋值给train_gt，用于后续存储训练集的标签。
    test_gt = np.zeros_like(gt)
    if train_size > 1:
       train_size = int(train_size)     #当训练样本比例大于1时，取整

    train_label = []
    test_label = []
    if mode == 'random':  #模式为随机
        if train_size == 1:   #对全部样本进行处理
            random.shuffle(X)   #使用random.shuffle()函数随机打乱数据集X的顺序，以确保训练集和测试集的数据随机分布。
            train_indices = [list(t) for t in zip(*X)]   #将打乱后的数据集X重新组织为训练集的索引列表。
            [train_label.append(i) for i in gt[tuple(train_indices)]]   #使用train_indices索引原始标签数组gt，将训练集的标签信息保存在train_label列表中。
            train_set = np.column_stack((train_indices[0],train_indices[1],train_label))   #将训练集的索引和对应的标签合并为一个二维数组train_set，其中第一列和第二列是训练样本的x和y坐标，第三列是对应的类别标签。
            train_gt[tuple(train_indices)] = gt[tuple(train_indices)]  #将训练集的索引对应位置上的原始标签信息复制到训练集标签数组train_gt中。
            test_gt = []  #清空
            test_set = []
        else:    #挑选部分样本进行处理
            train_indices, test_indices = sklearn.model_selection.train_test_split(X, train_size=train_size, stratify=y, random_state=23)   #使用Scikit-learn库中的train_test_split函数，将数据集X按照指定的训练集比例train_size划分为训练集和测试集。参数stratify=y表示按照y的类别分层抽样，保持训练集和测试集中各类别的比例与原始数据集中的比例相同，random_state=23用于设置随机种子，以便可重复的随机结果。
            train_indices = [list(t) for t in zip(*train_indices)]    #将训练集的索引列表重新组织为行列表。
            test_indices = [list(t) for t in zip(*test_indices)]     #将测试集的索引列表重新组织为行列表。
            train_gt[tuple(train_indices)] = gt[tuple(train_indices)]  #将训练集的索引对应位置上的原始标签信息复制到训练集标签数组train_gt中。
            test_gt[tuple(test_indices)] = gt[tuple(test_indices)]    #将测试集的索引对应位置上的原始标签信息复制到测试集标签数组test_gt中。

            [train_label.append(i) for i in gt[tuple(train_indices)]]   #使用train_indices索引原始标签数组gt，将训练集的标签信息保存在train_label列表中。
            train_set = np.column_stack((train_indices[0],train_indices[1],train_label))   #将训练集的索引和对应的标签合并为一个二维数组train_set，其中第一列和第二列是训练样本的x和y坐标，第三列是对应的类别标签。
            [test_label.append(i) for i in gt[tuple(test_indices)]]   #使用test_indices索引原始标签数组gt，将测试集的标签信息保存在test_label列表中。
            test_set = np.column_stack((test_indices[0],test_indices[1],test_label))   #将测试集的索引和对应的标签合并为一个二维数组test_set，其中第一列和第二列是测试样本的x和y坐标，第三列是对应的类别标签。
    ...
    return train_gt, test_gt, train_set, test_set          #返回训练样本标签 测试样本标签 训练样本组 测试样本组
