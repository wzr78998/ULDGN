...
class ConcatFusion(nn.Module):
    def __init__(self, input_dim=1024, output_dim=100):
        ...
        self.fc_out = nn.Linear(int(input_dim), int(output_dim))
...
class Shapley(nn.Module):
    ...
    def forward(self, audio, visual, drop = None, drop_arg = None,device= None):
        conv_layer = torch.nn.Conv2d(in_channels=visual.shape[1], out_channels=3, kernel_size=1).to(device)
        visual = conv_layer(visual)
        ...
        if drop_arg != None:
            if self.__dict__['training'] and drop_arg.warmup == 0:
                ...
                self.update = torch.Tensor(self.update).to(device)
                out = torch.cat((a,v),1)
